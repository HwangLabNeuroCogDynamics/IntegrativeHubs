...
if __name__ == "__main__":
    ...
    for s in [included_subjects]:
        ...
        coef = np.load(data_dir + "%s_" %s + coef_fn + "_coef.npy")
        ...
        # load models
        models = ["context", "color", "shape", "task", "stim", "response"]
        regressors = {}
        for m in models:
        ...
        for r in np.arange(num_ROIs):
            print("now running ROI number: ", r)
            # build dataframe
            df = pd.DataFrame()
            ...
            #run regression
            model = smf.ols(formula="coef ~ 1 + context + color + shape + stim + response", data=df).fit()
            #print(model.summary())
