...
try:
    searchlight_context = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=16, verbose=0, cv=LeaveOneGroupOut())
...
try:
    ...
    searchlight_color = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=16, verbose=0, cv=LeaveOneGroupOut())
...
try:
    pipeline = make_pipeline(StandardScaler(), LogisticRegression('l2', solver='liblinear')) 
    searchlight_shape = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=16, verbose=0, cv=LeaveOneGroupOut())
...
try:
    pipeline = make_pipeline(StandardScaler(), LogisticRegression('l2', solver='liblinear')) 
    searchlight_task = nilearn.decoding.SearchLight(mask, process_mask_img=mask, estimator = pipeline, radius=6, n_jobs=16, verbose=0, cv=LeaveOneGroupOut())
